...
from transformers import FlavaModel, FlavaProcessor, FlavaConfig, AutoProcessor, VisualBertForVisualReasoning, VisualBertConfig, Kosmos2Model
import torch
...
class Blip2VicunaDrive(Blip2Base):
    ...
    def __init__(
        ...
        if load_pretrained:
            ...
            self.visual_encoder.load_state_dict(pretrain_weights, strict=False)
        ...
        elif 'git' in llm_model:  # Add this condition
            self.llm_tokenizer = AutoTokenizer.from_pretrained("microsoft/git-base-coco", use_fast=False, truncation_side="left")
            self.llm_model = AutoModelForCausalLM.from_pretrained("microsoft/git-base-coco", torch_dtype=torch.float16,low_cpu_mem_usage=True,
                                                        trust_remote_code=True)
        elif 'flava' in llm_model:
            # Load three models: Image,Text and Multimodal
            self.llm_model = FlavaModel.from_pretrained(llm_model)
            self.llm_tokenizer = AutoProcessor.from_pretrained(llm_model)

        elif 'visBert' in llm_model:
            self.llm_tokenizer = AutoTokenizer.from_pretrained("google-bert/bert-base-uncased", use_fast=False, truncation_side="left")
            # self.llm_model = VisualBertForVisualReasoning.from_pretrained("uclanlp/visualbert-nlvr2",torch_dtype=torch.float16,low_cpu_mem_usage=True,
            #                                             trust_remote_code=True)
            self.llm_model = VisualBertForQuestionAnswering.from_pretrained("uclanlp/visualbert-nlvr2",
                                                                          torch_dtype=torch.float16,
                                                                          low_cpu_mem_usage=True,
                                                                          trust_remote_code=True)
            #self.llm_model.visual_bert.embeddings.visual_projection = torch.nn.Linear(768, 768)
            #config = VisualBertConfig.from_pretrained("uclanlp/visualbert-nlvr2")
            #config.visual_embedding_dim = 1024
            #self.llm_model = VisualBertForVisualReasoning.from_pretrained( "uclanlp/visualbert-nlvr2",config=config,
                                                                           #torch_dtype=torch.float16,
                                                                           #low_cpu_mem_usage=True,trust_remote_code=True)

        elif 'data2Vec' in llm_model:
            self.llm_tokenizer = AutoTokenizer.from_pretrained("facebook/data2vec-text-base", use_fast=False, truncation_side="left")
            self.llm_model = AutoModelForCausalLM.from_pretrained("facebook/data2vec-text-base", torch_dtype=torch.float16,low_cpu_mem_usage=True,
                                                         trust_remote_code=True)
        else:
        ...
        # This is updated to match the modifications of the previous step
        self.llm_model.resize_token_embeddings(len(self.llm_tokenizer))

        # This is not activated with the current config
    ...
    def forward(self, samples, inference_mode=False, image_embeds=None):
        ...
        # Only use concat function if LLM is baseline
        ...
        if "git" in self.llm_model_name:
            with self.maybe_autocast():
                hidden_states,_ = self.llm_model(
                    inputs_embeds=llm_inputs,
                    attention_mask=llm_attention_mask,
                    return_dict=False)

        elif "visBert" in self.llm_model_name:
            with self.maybe_autocast():
                hidden_states, = self.llm_model(inputs_embeds = llm_inputs,
                                               attention_mask = llm_attention_mask,
                                               return_dict = False)

        else:
            with self.maybe_autocast():
                hidden_states = self.llm_model(inputs_embeds = inputs_embeds,
        ...
        if self.has_gru_decoder:
            ...
            print("Calling Waypoints predictor")
            print(f"hidden_states device: {hidden_states.device}")
            print(f"Model hidden size: {self.llm_model.config.hidden_size}")
            print(f"Model hidden states: {hidden_states.shape}")
            proj_device = hidden_states.device
            #This will require a conditional statement
            if "git" in self.llm_model_name:
                projection_layer = nn.Linear(30523, self.llm_model.config.hidden_size).to(proj_device)
                hidden_states = projection_layer(hidden_states)
            elif "visBert" in self.llm_model_name:
                hidden_states = hidden_states.unsqueeze(1)
                hidden_states = hidden_states.repeat(1, seq_len, 1)
                print(f"hidden states after operation {hidden_states.shape}")
                projection_layer = nn.Linear(2, self.llm_model.config.hidden_size).to(proj_device)
                hidden_states = projection_layer(hidden_states)
