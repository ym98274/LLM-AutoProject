...
from .resnet import resnet26d, resnet50d, resnet18d, resnet26, resnet50, resnet101d, resnet34d, resnext50d_32x4d
from .cspnet import cspdarknet53
from .layers import StdConv2dSame, StdConv2d, to_2tuple
from .pointpillar import PointPillarNet, ConvBackbone
from .dgcnn import DGCNN
from transformers import ConvNextConfig, ConvNextModel
# from .voxelnet import SVFE, CML
...
class HybridEmbed(nn.Module):
    def __init__(
        ...
        if feature_size is None:
            with torch.no_grad():
                ...
                # Use a dummy input to determine feature size
                dummy_input = torch.zeros(1, in_chans, img_size[0], img_size[1]).cuda()
                o = self.backbone(dummy_input)

                # Handle different types of outputs
                if isinstance(o, (list, tuple)):
                    o = o[-1]  # Last feature if backbone outputs list/tuple of features
                elif hasattr(o, 'last_hidden_state'):
                    o = o.last_hidden_state  # Access last_hidden_state if available
                elif isinstance(o, torch.Tensor):
                    pass  # If the output is already a tensor
                else:
                    raise TypeError(f"Unexpected output type: {type(o)}")
        ...
        # Convolutional layer to project the backbone features to the embedding dimension
    ...
    def forward(self, x):
        ...
        if isinstance(x, (list, tuple)):
            ...
            x = x.last_hidden_state  # Access last_hidden_state if available
        elif isinstance(x, torch.Tensor):
            pass  # If the output is already a tensor
        else:
            raise TypeError(f"Unexpected output type: {type(x)}")

        # Pass the extracted tensor through the projection layer
        x = self.proj(x)
...
def to_2tuple(x):
    # Helper function to ensure inputs are 2-tuples
...
class dgcnnModel(nn.Module):
    def __init__(self, k=2, emb_dims=512,
                 dropout=0.5, output_channels=256):
        super().__init__()

        self.dgcnn = DGCNN(k,emb_dims,dropout,output_channels)

    def forward(self, lidar):
        print("Start of forward pass:")
        print(f"Shape before transpose {lidar.shape}")
        features = lidar.transpose(1,2)
        print(f"Shape after transpose {features.shape}")
        features = self.dgcnn(features)
        print(f"Shape after dgcnn {features.shape}")
        return features
...
class Memfuser(nn.Module):
    def __init__(
        ...
        elif rgb_backbone_name == "rx50":
            self.rgb_backbone = resnext50d_32x4d(
                pretrained=True,
                in_chans=in_chans,
                features_only=True,
                out_indices=[4],
            )
        elif rgb_backbone_name == "dn53":
            self.rgb_backbone = cspdarknet53(
                pretrained=True,
                in_chans=in_chans,
                features_only=True,
                out_indices=[4],
        ...
        elif rgb_backbone_name == "convNext":
            convnext_config = ConvNextConfig(
                num_channels=3,  # Number of input channels (RGB images)
                patch_size=4,  # Patch size for embedding
                num_stages=4,  # Number of stages in the model
                hidden_sizes=[96, 192, 384, 768],  # Hidden sizes at each stage
                depths=[3, 3, 9, 3],  # Depth of blocks in each stage
                ...
                out_indices=[3]  # Indices of features to output (last stage)
            )

            self.rgb_backbone = ConvNextModel.from_pretrained("facebook/convnext-tiny-224",
                                                              config=convnext_config)

        if lidar_backbone_name == "pillars":
        ...
        elif lidar_backbone_name == "dgcnn":
            self.lidar_backbone = dgcnnModel()
            #self.lidar_backbone = torch.nn.DataParallel(self.lidar_backbone)
    ...
    def forward_features(
        ...
        if self.lidar_backbone_name == "pillars":
            lidar_token = self.lidar_backbone(lidar, num_points)
        elif self.lidar_backbone_name == "dgcnn":
            lidar_token = self.lidar_backbone(lidar)
        ...
        #print(f"Shape after position encoding: {lidar_token.shape}")
        lidar_token = lidar_token.flatten(2).permute(2, 0, 1)
        #print(f"Final lidar_token shape: {lidar_token.shape}")
...
@register_model
def resnext_baseline(**kwargs):
    model = Memfuser(
        enc_depth=1,
        dec_depth=3,
        embed_dim=256,
        rgb_backbone_name="rx50",
        lidar_backbone_name="pillars",
        waypoints_pred_head="gru",
    )
    return model
...
@register_model
def resnext_return_feature(**kwargs):
    '''
    _return_feature means the model only output the intermediate features,
        and it does not need to take as inputs the target waypoint.
    '''
...
@register_model
def convnext_baseline(**kwargs):
    model = Memfuser(
        enc_depth=1,
        dec_depth=3,
        embed_dim=256,
        rgb_backbone_name="convNext",
        lidar_backbone_name="pillars",
        waypoints_pred_head="gru",
    )
    return model
...
@register_model
def convnext_return_feature(**kwargs):
    '''
    _return_feature means the model only output the intermediate features,
        and it does not need to take as inputs the target waypoint.
    '''
...
@register_model
def cspdn53_baseline(**kwargs):
    model = Memfuser(
        enc_depth=1,
        dec_depth=3,
        embed_dim=256,
        rgb_backbone_name="dn53",
        lidar_backbone_name="pillars",
        waypoints_pred_head="gru",
    )
    return model
...
@register_model
def cspdn53_return_feature(**kwargs):
    '''
    _return_feature means the model only output the intermediate features,
        and it does not need to take as inputs the target waypoint.
    '''
...
# CUSTOM LIDAR MODELS
# THESE ARE TESTED WITH A RESNET50 ENCODER

@register_model
def dgcnn_baseline(**kwargs):
    model = Memfuser(
        enc_depth=1,
        dec_depth=3,
        embed_dim=256,
        rgb_backbone_name="r50",
        lidar_backbone_name="dgcnn",
        waypoints_pred_head="gru",
    )
    return model

@register_model
def dgcnn_baseline_return_feature(**kwargs):
    '''
    _return_feature means the model only output the intermediate features,
        and it does not need to take as inputs the target waypoint.
    '''
